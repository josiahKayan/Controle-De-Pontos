...
namespace Apassos.DataAccess
{
    ...
    public void SalvarUsuario(Users usuario, string checkPerfil, string idParceiro, Users usuarioLogado, string locked, string isalterpwd)
    {
      ...
      */
        public  List<UserPerfil> GetUserPerfisUsuario(Users usuario)
        {
            using (TimesheetContext db = new TimesheetContext())
            {
                var env = ConfigurationManager.AppSettings["ENVIRONMENT"].ToString();
      ...
      */
        ...
        public  bool VerificaPerfil(Users usuario, Perfil perfil)
        {
            using (TimesheetContext db = new TimesheetContext())
            {
        ...
        public  string CreateLogin(Partners partner)
        {
            using (TimesheetContext db = new TimesheetContext())
            {
        ...
        public string SenhaDefault(Users usuario)
        {
            return (usuario.USERNAME.ToLower() + ":" + "@passos".ToLower()).sysPassEncrypt();
        }
        public  string SenhaDefault(String login)
        {
            return (login.ToLower() + ":" + "@passos".ToLower()).sysPassEncrypt();
        }
        public  string ConcatSenhaAcesso(string login, string senha, bool cript)
        {
