    ...
    public String Entry(string login, string pass, string desc, string idts)
    {
      ...
      try
      {
        ...
        if (UsersDataAccess.isLogin(login, pass))
        {
          ...
          Period periodoAtual = PeriodDataAccess.GetPeriodoAtual();
    ...
    public String starttsChecks(string login, string pass, string desc, string idts)
    {
      try
      {
        ...
        if (UsersDataAccess.isLogin(login, pass))
        {
          ...
          Period periodoAtual = PeriodDataAccess.GetPeriodoAtual();
          Partners consultorAtual = PartnerDataAccess.GetPartnerByLogin(login);
          if (!TimesheetDataAccess.isApontamentoByStartTS(login))
          {
            TimesheetHeader timesheetHeaderSave = TimesheetDataAccess.GetApontamentoCabecalhoPorPeriodo(consultorAtual, periodoAtual);
            List<ProjectUser> projetosconsultor = ProjectDataAccess.GetProjetosPorConsultor(consultorAtual, true);
            List<TimesheetItem> listTimesheetItem = TimesheetDataAccess.GetiTensApontamentoPorCabecalho(timesheetHeaderSave);
            if (timesheetHeaderSave != null && (projetosconsultor != null && projetosconsultor.Count() > 0))
            {
              ...
              string _out = "18:00";
              string _break = "01:30";
              TimesheetDataAccess.SalvarItemApontamento(null, _data, _projectid, _type, _in, _out, _break, _description, timesheetHeaderSave.TIMESHEETHEADERID.ToString(), consultorAtual.PARTNERID.ToString(), periodoAtual.PERIODID.ToString());
              var ip = Request.UserHostAddress;
              Util.EscreverLog("[STSIns][" + login + ";" + desc + "]", ip + ":" + login, "startts.log");
            }
            return "Insert OK";
          ...
          {
            TimesheetItem it = TimesheetDataAccess.GetApontamentoByStartTS(login);
            string _data = it.DATE.ToString("dd/MM/yyyy");
            string _projectid = it.project.PROJECTID.ToString();
            string _type = it.TYPE;
            string _in = it.IN.ToString().Substring(0, 5);
